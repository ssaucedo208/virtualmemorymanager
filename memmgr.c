...
unsigned getframe(FILE* fstore, unsigned logic_add, unsigned page,
 ...
 if(page_table[page] != -1)
  ...
  int avail = get_available_frame(page);
  fread(&main_mem[avail * FRAME_SIZE], sizeof(char), FRAME_SIZE, fstore);
  page_table[page] = avail;
  update_tlb(page);
  return -1;
}
...
unsigned getframe_fifo(FILE* fstore, unsigned logic_add, unsigned page,
  ...
  int index2 = tlb_contains(page);
  // tlb hit
  if(index2 != -1)
  {
    if(page_queue[tlb[index2][1]] == page)
    {
      printf("TLB hit\n");
      (*tlb_hit_count)++;
      return tlb[index2][1];
  ...
  // bring data into memory, update tlb and page table
  int avail = get_available_frame(page);
  fread(&main_mem_fifo[avail * FRAME_SIZE], sizeof(char), 128, fstore);
  page_table[page] = avail;
  update_tlb(page);
  return -1;
}
